...
\Pr&[B(t) \ge 8nln(n)] \le \frac{ln(n) + c}{8ln(n)} \overset{?}{\le} 0.25 \\
\end{align*}

\textbf{Lets simplify this further}
\begin{align*}
\frac{ln(n) + c}{8ln(n)} &\overset{?}{\le} 0.25 \\
ln(n) + c &\overset{?}{\le} {0.25 * 8ln(n)} \\
ln(n) + c &\overset{?}{\le} {2ln(n)} \iff c \le ln(n)\\
\end{align*}
...
\textbf{We first start by defining our rouding criteria. If $x_u > 0.8, y_u = 1$. Otherwise $y_u = 0$}

\textbf{Let $X = x_1 + x_2 + ... + x_n$ represent the optimal solution.}

\textbf{Let $Y = y_1 + y_2 + ... + y_n$ represent the rounded solution.}

\textbf{Because of the interval given $y_u = 1 \iff x_u > 0.8$}

\textbf{Consider the lowest value of $X$ that can be cause $y_u = 1$. This value is $ > 0.80$. This gives the innequality $y_u < 1.25 * x_u$}
