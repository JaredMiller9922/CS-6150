...
\textbf{This question depends on what we mean by "reasonable certainty". I think that a good start would be 10000 tosses. This might be overkill but I believe it will make the noise have less of an effect.}
\part[2] Suppose we pick $N = 25$. What is the expected value of $H_2 - H_1$?
\textbf{To solve this problem we first realize it can be setup as a bernoulli trial. We know that the expected value of a bernoulli trial is simply the probability of success. $E[X1] = 0.50$ and $E[X2] = 0.51$ for each trial. With this knowledge we can also calculate the probability after any number of trials.} 

\textbf{$E[H_1] = x_1, x_2, ..., x_n$} = $N * p1$

\textbf{$E[H_2] = x_1, x_2, ..., x_n$} = $N * p2$

\textbf{We can then use linearity of expectation to calculate $E[h_2 - h_1] = (0.51 * 25) - (0.5 * 25) = 0.25$}
