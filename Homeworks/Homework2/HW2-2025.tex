...
\textbf{Because we know that our solution is a local optimum this innequality shows that the choice that we made for the first element of the list $q1$ must be $\le$ the choice that was made for the optimal $x1$. Swapping $q1$ for $x1$ here won't provide a better solution otherwise we would have swapped. Therefore the optimal must be $le$ the choice that we made (Remembering that we are trying to maximize the distance)}
\mysolution{
\input{Sol-2a}
}
...
\[(t-1)d(x_1,x_2) \le \sum_{j=2}^td(x_1, q_j) + \sum_{j=2}^td(q_j, x_2)\]
...
\textbf{Simply plugging in those new innequalities we get the innequality asked for. We are okay to plug these in because we know they are strictly greater than so we don't ruin the innequality at the top}
\[ (t-1) \cdot d(x_1, x_2) \le 2 \left[ d(q_1, q_2) + d(q_1, q_3) + \dots + d(q_1, q_t) \right]. \]
...
\textbf{The argument for (b) can be applied to all pairs of points in the optimum}
Note: this shows that the local optimum has an objective value at least $\nicefrac{1}{2o
}$ the global optimum.
\textbf{The main idea is to generalize what we got in (b) to all terms. All of the arguments from a and b can be applied to all values in the optimal solution}
\[(t-1)\sum_{i,j}d(x_i,x_j) \le 2 * (t-1)\sum_{i,j}d(q_i,q_j)\]
\textbf{The value of (t-1) is just a constant so it can be factored out of each element of the sum. We also realize that each term is counted twice on the right hand using the two equations from (a). We know that there are $t-1$ terms and each of those terms are counted twice which is where the extra $(t-1)$ comes from on the left hand side.} 
\[ d(x_1, q_2) + d(x_1, q_3) + \dots + d(x_1, q_t) \le d(q_1, q_2) + d(q_1, q_3) + \dots + d(q_1, q_t). \]
\[ d(x_2, q_2) + d(x_2, q_3) + \dots + d(x_2, q_t) \le d(q_2, q_1) + d(q_2, q_3) + \dots + d(q_2, q_t). \]
\textbf{Simplify by removing the $(t-1)$ from both sides to get the answer}
\[ \sum_{i,j} d(x_i, x_j) \le 2 \sum_{i,j} d(q_i, q_j).\]
\mysolution{
\input{Sol-2c}
}
