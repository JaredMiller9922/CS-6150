...
\textbf{Because I needed diagrams to show my work I have included some handwritten notes for my work.}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\linewidth]{4a.jpg}
...
\textbf{We can actually quickly see that this will still be linear by simply looking at the coeficients inside of our $T()$ portions of the function. $n\frac{8}{11} = 0.72n$ and $n\frac{1}{11} = 0.09n$. These don't add up to $1$ so we know its still linear. We can also formally solve the recurrence using the plug and chug method. Using this method we find that the amount of extra work we are doing at each level is $\frac{9}{11}^{n} * c$. This gives us a geometric series that can be simplified to $5.5cn$ which is still linear!. Detailed work of the plug and chug can be seen in the screenshot below.}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\linewidth]{4b.jpg}
  \caption{Showing The Recurrence Is Linear}
\end{figure}
...
\textbf{We can use the same lane of understanding that was used above. $n\frac{4n}{5} = 0.8n$ and $n\frac{n}{5} = 0.2n$ This means that at each step we are solving $n$ size problem. This ends up being $O(nlog(n))$ like mergesort}
\mysolution{
\input{Sol-4c}
}
\end{parts}
...
\textbf{To solve this problem we will use the plug and chug method. The work can be seen in the screenshot below. Once we solve the reccurence we get the following runtime $O(n)$.}
...
\textbf{We can also solve this using plug and chug. The work can be seen in the screenshot below. Once we solve the reccurence we get the following runtime $O()$.}
...
$\displaystyle \operatorname*{argmin}_{S,W,C}\sum_{i=0}^{S}(W + C * \text{line\_fit}(s_i))$
...
\textbf{The algorihtm I designed can be seen in the screenshot below.}

\textbf{Note: I use a simple array as my lookup table and index into that lookup table using the index of the current house we are looking at.}

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\linewidth]{water_pipeline_alg.png}
    \caption{Algorithm For Finding the Minimum Cost Pipeline}
    \label{fig:water_pipeline}
\end{figure}
\mysolution{
\input{Sol-6b}
}
...
\textbf{At each step of the recursion we compute the cost if we connected the rest of the nodes with a single segment and the cost if we created a segment and continued on the recursion. Both of these things happen for every node $O(n)$ times overall. Because we can assume our oracle is constant the final runtime is O(n)}
...
\textbf{Dyanmic programming is often useful for problems where we need to try every possibility and have multiple recursive branches. We notice above that a huge part of us being able to get the runtime specified is because we know that we only have to solve every subproblem once. If we hadn't done that every single branch would have to resolve the subproblem.}
